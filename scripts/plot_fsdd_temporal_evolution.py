...
MODEL_SPECS = {
    ...
    "Conv1D": {
        "history": ROOT / "outputs" / "fsdd_conv1d" / "metrics" / "history.csv",
        "diagnostics": ROOT
        / "outputs"
        / "fsdd_conv1d"
        / "metrics"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare pooled EGFN, Temporal EGFN, and Conv1D on FSDD."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "reports" / "figures" / "fsdd_temporal_evolution",
    )
...
def plot_comparison(
    ...
    curve_axis.set_title("(a) Speaker-disjoint validation", loc="left", fontweight="bold")
    curve_axis.set_xlabel("Epoch")
    curve_axis.set_ylabel("Validation accuracy")
    curve_axis.set_ylim(0.25, 0.78)
    ...
    bar_axis.set_title("(b) Unseen-speaker test", loc="left", fontweight="bold")
    bar_axis.set_ylabel("Test accuracy")
    bar_axis.set_ylim(0.0, 0.84)
    bar_axis.tick_params(axis="x", labelrotation=15)
    ...
    for column, (model_name, panel_letter) in enumerate(
        ...
        normalized = np.asarray(diagnostics[model_name]["normalized_confusion"])
    ...
    figure.savefig(output_dir / "fsdd_temporal_evolution.png", dpi=300)
    figure.savefig(output_dir / "fsdd_temporal_evolution.pdf")
    plt.close(figure)
...
def main() -> None:
    ...
    save_summary(args.output_dir / "fsdd_temporal_summary.csv", histories, diagnostics)
    ...
    print(f"saved_figure={args.output_dir / 'fsdd_temporal_evolution.png'}")
    print(f"saved_pdf={args.output_dir / 'fsdd_temporal_evolution.pdf'}")
